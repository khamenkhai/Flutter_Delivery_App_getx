...
class _DeliveryDetailScreenState extends State<DeliveryDetailScreen> {
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        bottom: PreferredSize(
            ...
            child: Obx(() => driverDeliveryController.loading.value
  ...
  void setDeliveryStatus(
    ...
    if (widget.delivery.status == DeliveryStatus.pending) {
      bool status = await driverDeliveryController.setDeliveryStatus(
        orderId: widget.delivery.orderId.toString(),
        status: DeliveryStatus.active,
        deliveryId: widget.delivery.deliveryId.toString(),
      );
      if (status) {
        Navigator.pop(context);
      }
    } else if (widget.delivery.status == DeliveryStatus.active) {
      showModalBottomSheet(
        context: context,
        builder: (context) {
          return CompleteDeliveryButtonSheet(
              customerReceived: widget.customerReceived,
              signatureController: controller,
              onSignatureConfirmed: () async {
                userSignatureFile = await controller.toPngBytes();

                if (userSignatureFile != null) {
                  bool status = await driverDeliveryController
                      .setDeliveryStatus(
                          status: DeliveryStatus.complete,
                          orderId: widget.delivery.orderId.toString(),
                          deliveryId: widget.delivery.deliveryId.toString(),
                          file: userSignatureFile)
                      .then((value) {
                    Navigator.pop(context);
                    return value;
                  });
...
// ignore: must_be_immutable
class CompleteDeliveryButtonSheet extends StatefulWidget {
  CompleteDeliveryButtonSheet(
      {super.key,
  ...
  final Function onSignatureConfirmed;
  @override
  State<CompleteDeliveryButtonSheet> createState() =>
      _CompleteDeliveryButtonSheetState();
}

class _CompleteDeliveryButtonSheetState
    extends State<CompleteDeliveryButtonSheet> {
  @override
  Widget build(BuildContext context) {
        ...
        children: [
          ...
          Container(
            ...
            child: Row(
              children: [
                Checkbox(
                    value: widget.customerReceived,
                    checkColor: Colors.white,
                    activeColor: Colors.black,
                    onChanged: (value) {
                      setState(() {
                        widget.customerReceived = value!;
                      });
                    }),
                Text("User already received")
              ],
            ),
          ),
          widget.customerReceived
              ? Container(
                  height: 300,
                  width: 300,
